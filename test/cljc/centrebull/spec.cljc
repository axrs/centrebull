(ns cljc.centrebull.spec
  #?(:clj
     (:import java.util.UUID))
  (:require
    [clojure.test :refer [deftest testing is]]
    [jesi.test.util :refer [is-not]]
    [clojure.spec :as s]
    [jesi.spec.util :as u]
...
(deftest test->uuid
  (testing "Converts the first parameter to a UUID object, or returns ::s/invalid"
    (is (uuid? (u/->uuid "2c80c3ca-535c-4706-bea2-afd2a2bf374d")))
    #?(:clj  (is (uuid? (u/->uuid (UUID/randomUUID))))
       :cljs (is (uuid? (u/->uuid (UUID/make-random)))))
    (is (s/invalid? (u/->uuid "1234")))
    (is (s/invalid? (u/->uuid 1234)))
    (is (s/invalid? (u/->uuid nil)))))

(def ^:private non-empty-string #'centrebull.spec/non-empty-string)
